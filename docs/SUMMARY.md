* [介绍](README.md)

### Java笔记

* [Android笔记](Android笔记/README.md)
	* [Activity布局](Android笔记/Activity布局.md)
	* [Android 蓝牙通信](Android笔记/Android 蓝牙通信.md)
	* [Android 调试](Android笔记/Android 调试.md)
	* [Handle初识(1)](Android笔记/Handle初识(1).md)
	* [Handle初识(2)](Android笔记/Handle初识(2).md)
	* [Handle初识(3)](Android笔记/Handle初识(3).md)
	* [SQLite Android开发](Android笔记/SQLite Android开发.md)
	* [Activity Intent](Android笔记/activity-intent.md)
	* [Android常用控件](Android笔记/android常用控件.md)
	* [Menu](Android笔记/menu.md)
	* [Note1](Android笔记/note1.md)

* [Java基础](Java笔记/README.md)
	* [Collection 集合类](Java笔记/Collection-集合类.md)
	* [DTO](Java笔记/DTO.md)
	* [InputStream和Reader的根本差异](Java笔记/InputStream和Reader的根本差异.md)
	* [Java IO流](Java笔记/JavaIO流.md)
	* [Java 变量存储理解](Java笔记/Java变量存储理解.md)
	* [Java单例设计模式](Java笔记/Java单例设计模式.md)
	* [Java字节字符转换流](Java笔记/Java字节字符转换流.md)
	* [Java正则表达式](Java笔记/Java正则表达式.md)
	* [List遍历：For，Foreach还是Iterator](Java笔记/List遍历：for，foreach还是Iterator.md)
	* [Service的配置](Java笔记/Service的配置.md)
	* [String,StringBuffer与StringBuilder的区别](Java笔记/String,StringBuffer与StringBuilder的区别.md)
	* [StringBuffer](Java笔记/StringBuffer.md)
	* [StringBuilder](Java笔记/StringBuilder.md)
	* [String类](Java笔记/String类.md)
	* [Bean](Java笔记/bean.md)
	* [Maven打包](Java笔记/maven打包.md)
	* [Web.Xml文件的配置简析](Java笔记/web.xml文件的配置简析.md)
	* [事务配置](Java笔记/事务配置.md)
	* [停止线程的三种方式](Java笔记/停止线程的三种方式.md)
	* [多线程 锁的理解](Java笔记/多线程锁的理解.md)
	* [多线程小结](Java笔记/多线程小结.md)
	* [字符串常用操作](Java笔记/字符串常用操作.md)
	* [字符串常用方法](Java笔记/字符串常用方法.md)
	* [字符串编码研究](Java笔记/字符串编码研究.md)
	* [字节流复制文件](Java笔记/字节流复制文件.md)
	* [成员变量与局部变量理解及Static变量](Java笔记/成员变量与局部变量理解及static变量.md)
	* [线程间通信 等待唤醒机制](Java笔记/线程间通信-等待唤醒机制.md)
	* [自定义的BufferedReader](Java笔记/自定义的BufferedReader.md)

* [Java框架](框架/README.md)
	* [Action 配置](框架/Action配置.md)
	* [Ibatis+Mybatis](框架/Ibatis+Mybatis/README.md)
		* [IBATIS 框架 学习小结](框架/Ibatis+Mybatis/iBATIS框架学习小结.md)
		* [IBATIS 框架 配置详解](框架/Ibatis+Mybatis/iBATIS框架配置详解.md)
		* [Ibatis 映射文件中标签详解](框架/Ibatis+Mybatis/ibatis映射文件中标签详解.md)
	* [SSI框架学习小结](框架/SSI框架学习小结.md)
	* [SpringBean的注入](框架/springBean的注入.md)
	* [简单Spring项目理解Spring的依赖注入](框架/简单Spring项目理解Spring的依赖注入.md)


### 运维笔记

* [Linux笔记](Linux笔记/README.md)
	* [Linux备份](Linux笔记/Linux备份.md)
	* [Linux常用命令](Linux笔记/Linux常用命令.md)
	* [Linux环境变量配置](Linux笔记/Linux环境变量配置.md)
	* [Linux防火墙](Linux笔记/Linux防火墙.md)
	* [WSL](Linux笔记/WSL.md)
	* [Linux使用问题](Linux笔记/linux使用问题.md)

* [arthas简单使用](arthas/简单使用.md)

### 中间件

* [Tomcat](中间件/tomcat/README.md)
    * [Tomcat JNDI数据源](中间件/tomcat/TomcatJNDI数据源.md)
    * [Tomcat内存设置](中间件/tomcat/tomcat内存设置.md)
	* [Tomcat Https 访问](服务器SSL/tomcat-Https-访问.md)
	
* [Weblogic](中间件/weblogic/README.md)
    * [WebLogic问题记录](中间件/weblogic/WebLogic问题记录.md)
    * [数据源连接池设置](中间件/weblogic/数据源连接池设置.md)
	
* [WebLogic与Tomcat处理Response.GetWriter()的不同处理](中间件/记一次WebLogic与tomcat处理response.getWriter()的不同处理.md)
* [远程调试](中间件/远程调试.md)

### 数据库

* [MySQL笔记](MySQL笔记/README.md)
	* [MySQL与Oracle](MySQL笔记/MySQL与Oracle.md)
	
* [Oracle](Oracle/README.md)
	* [Oracle学习笔记(2)](Oracle/Oracle学习笔记(2).md)
	* [Oracle学习笔记](Oracle/Oracle学习笔记.md)
	* [PLSQL](Oracle/PLSQL.md)
	* [Oracle培训](Oracle/oracle培训.md)

* [不常用Sql](SQL/不常用sql.md)

* [分库分表](分库分表/README.md)
	* [MyCat使用记录](分库分表/MyCat使用记录.md)
	* [ShardingJdbc使用记录](分库分表/ShardingJdbc使用记录.md)
	* [分库分表实践](分库分表/分库分表实践.md)

### Ta框架

* [Ta3](Ta3/README.md)
	* [Ta3Cloud Tips](Ta3/Ta3Cloud-tips.md)
	* [Ta3不常用功能记录](Ta3/Ta3不常用功能记录.md)
	* [TaCache](Ta3/TaCache.md)
	* [Ta应用Echarts](Ta3/Ta应用Echarts.md)
	* [Ta应用润乾](Ta3/Ta应用润乾.md)
	* [Ta框架源码分析](Ta3/Ta框架源码分析.md)
	* [前台框架不支持采用原生或其他框架记录](Ta3/前台框架不支持采用原生或其他框架记录.md)

* [Ta404](Ta404/README.md)
	* [Ta404 使用问题记录](Ta404/ta404使用问题记录.md)
	* [Ta404 框架踩坑指南](Ta404/Ta404框架踩坑指南.md)

### 前端

* [前端](前端/README.md)
	* [CSS初识](前端/CSS初识.md)
	* [ECharts](前端/ECharts.md)
	* [ES6自学](前端/ES6自学.md)
	* [JavaScript初识](前端/JavaScript初识.md)
	* [前端开发问题记录](前端/前端开发问题记录.md)
	
* [Vue](前端/vue.md)
* [微信公众号开发教程](微信公众号/微信公众号开发教程.md)

### Elastic Job

* [Elastic Job Lite使用问题记录](elastic-job/elastic-job-lite使用问题记录.md)

### Zookeeper

* [Zookeeper使用记录](zookeeper/zookeeper使用记录.md)
* [Zookeeper监控](zookeeper/zookeeper监控.md)
* [Zookeeper集群配置](zookeeper/zookeeper集群配置.md)

### 缓存数据库

* [Redis问题记录](缓存数据库/Redis问题记录.md)

### 开发工具

* [IDEA](开发工具/IDEA.md)
* [SQL Developer不常用功能记录](开发工具/PLSQLDeveloper配置.md)

## 其它

* [Git操作](其它/Git操作.md)
* [SVN](其它/SVN.md)
* [Hello World](其它/hello-world.md)
* [Windows设置只有指定IP段才走VPN](其它/windows设置只有指定IP段才走VPN.md)
* [大O记号](其它/大O记号.md)
* [常用Markdown语法](其它/常用Markdown语法.md)

> * [阿里巴巴校招编程试题](其它/阿里巴巴校招编程试题.md)
> * [阿里巴巴校招编程试题2](其它/阿里巴巴校招编程试题2.md)
